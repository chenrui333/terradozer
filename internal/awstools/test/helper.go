...
// GetTerraformOptions returns options to run Terraform commands for tests.
// Note: tfStateBucket is the S3 bucket that stores all Terraform states of acceptance tests.
// The bucket must be located in "profile1" and "region1".
func GetTerraformOptions(tfStateBucket, terraformDir string, env Vars,
	overrideVars ...map[string]any) *terraform.Options {
	name := "testacc-" + strings.ToLower(random.UniqueId())
