// Package provider implements a client to call import, read, and destroy
// on any Terraform provider Plugin via GRPC.
package provider
...
type provider interface {
	Configure(req providers.ConfigureRequest) providers.ConfigureResponse
	GetSchema() providers.GetSchemaResponse
	ReadResource(req providers.ReadResourceRequest) providers.ReadResourceResponse
	ApplyResourceChange(req providers.ApplyResourceChangeRequest) providers.ApplyResourceChangeResponse
	ImportResourceState(req providers.ImportResourceStateRequest) providers.ImportResourceStateResponse
	Close() error
}
...
// Timeout is the amount of time to wait for a destroy operation of the provider to finish.
func Launch(ctx context.Context, pathToPluginExecutable string, timeout time.Duration) (*TerraformProvider, error) {
	m := discovery.PluginMeta{
		Path: pathToPluginExecutable,
	}

	p, err := providerFactory(ctx, m, hclog.Error)()
	if err != nil {
		return nil, err
	}
...
func providerFactory(ctx context.Context, meta discovery.PluginMeta, loglevel hclog.Level) providers.Factory {
	return func() (providers.Interface, error) {
		client := goPlugin.NewClient(clientConfig(ctx, meta, loglevel))
...
func clientConfig(ctx context.Context, m discovery.PluginMeta, loglevel hclog.Level) *goPlugin.ClientConfig {
	...
	return &goPlugin.ClientConfig{
		Cmd:              exec.CommandContext(ctx, m.Path), //nolint:gosec
...
func (p TerraformProvider) GetSchemaForResource(terraformType string) (providers.Schema, error) {
	...
	if !ok {
		return providers.Schema{}, errors.New("failed to get schema for resource")
	}
...
func Init(providerName string, installDir string, timeout time.Duration) (*TerraformProvider, error) {
	...
	if err != nil {
		log.WithField(logFieldName, providerName).Debug("ignoring resources of (yet) unsupported provider")
		return nil, nil //nolint:nilerr,nilnil // Unsupported providers are skipped rather than failing all resources.
	}
	...
	p, err := Launch(context.Background(), metaPlugin.Path, timeout)
	if err != nil {
	...
	log.WithFields(log.Fields{
		logFieldName:    metaPlugin.Name,
		logFieldVersion: metaPlugin.Version,
